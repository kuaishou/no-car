...
import { getYears, getMonths, getDays, getHours, getMinute } from './makeData'
...
DatetimePicker.prototype = {
      ...
      each(TYPE_MAP, function (type) {
        ...
        self[type + 'Scroller'] = renderScroller(div, data, trimZero(newValueMap[type]), function (currentValue) {
          ...
          if (type === 'year' || type === 'month' || type === 'day' || type === 'hour') {
            ...
            if (type == 'hour') {
              self.minuteScroller && self._setMinuteScroller(self.yearScroller.value, self.monthScroller.value, self.dayScroller.value, self.hourScroller.value, self.minuteScroller.value)
            }
          }
          let currentDay
          if (type === 'year') {
            ...
            if (self.dayScroller) {
              ...
              console.log('开始设置天==config.currentMonth==',currentValue,config.currentMonth)
              console.log('开始设置天====',currentValue,currentMonth)
              setTimeout(()=>{
                self._setDayScroller(currentValue, self.monthScroller.value, currentDay)
                console.log('延时=', self.monthScroller.value )
              },0)
             
            }
            self.hourScroller&&self._setHourScroller(currentYear, currentValue, currentDay, self.hourScroller.value)
            self.minuteScroller&&self._setMinuteScroller(currentYear, currentValue, currentDay,self.hourScroller.value,self.minuteScroller.value)
          } else if (type === 'month') {
            // debugger
            // console.log('月份滚动那个')
            const currentYear = self.yearScroller ? self.yearScroller.value : config.currentYear
  ...
  _makeData(type, year, month, day) {
    ...
    if (type === 'year') {
      ...
      if (this.reMakeData) {
        ...
        console.log('天数2=',minDay,maxDay)
        console.log('天数2=',min,max)
      }
    } else if (type === 'hour') {
      // console.log(this)
      ...
      //   this.getValues()
      // ))
      const { minHour, maxHour } = getHours(this.config.startDate, this.config.endDate, this.getValues())
      min = minHour
      // // min = this.config.minHour
      // max = this.config.maxHour
      max = maxHour
    } else if (type === 'minute') {
      const { minMinute, maxMinute } = getMinute(this.config.startDate, this.config.endDate, this.getValues())
  ...
  getValues() {
    ...
    let date = this.getValue().split(/-|\/|:|\s/)
  ...
  _setMonthScroller(currentValue, month) {
    ...
    self.monthScroller = renderScroller(div, self._makeData('month'), month, function (currentValue) {
      ...
      if (self.yearScroller && self.monthScroller && self.hourScroller && self.minuteScroller) {
        self._setMinuteScroller(currentYear, currentValue, self.dayScroller.value, self.hourScroller.value, self.minuteScroller.value)
      }
    })
  },

  _setDayScroller(year, month, day) {
    console.log('设置天=',year, month, day)
    if (!this.dayScroller) {
      return
    }
    const self = this
  ...
  _setMinuteScroller(year, month, day, hour, minute) {
    console.log('_setMinuteScroller')
    ...
    self.minuteScroller = renderScroller(div, self._makeData('minute', year, month, day), minute || '', function (currentValue) {
