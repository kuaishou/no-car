...
const passiveSupported = require('./passive_supported')
...
const Scroller = function (container, options) {
  ...
  if (data.length && data[0].constructor === Object) {
    data.forEach(function (row) {
      html += '<div class="' + self.options.itemClass + ' data-name=" ' + row.name + '" data-value=' + JSON.stringify({value: encodeURI(row.value)}) + '>' + row.name + '</div>'
    })
  } else {
    data.forEach(function (val) {
      html += '<div class="' + self.options.itemClass + '"  data-name=" ' + row.name + '" data-value=' + JSON.stringify({value: encodeURI(val)}) + '>' + val + '</div>'
    })
  }
  content.innerHTML = html

  self.__container.appendChild(component)
console.log('sss',(indicator))
console.log('sss',getComputedStyle(indicator, 'height'))


  self.__itemHeight = parseFloat(getComputedStyle(indicator, 'height'), 10)
  console.log('__itemHeight',self.__itemHeight)
